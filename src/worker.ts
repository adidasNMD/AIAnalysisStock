...
import { getDb, openDbConnection, setDbInstance } from './db';
...
(async () => {
  ...
  try {
    const migrateResult = await runMigrations(db);
    setDbInstance(db);
    logger.info(`[startup] migrations applied: ${migrateResult.applied.length}`, { applied: migrateResult.applied });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    logger.error(`[startup] schema checksum mismatch or migration failure: ${message}`);
    await db.close();
    process.exit(1);
  }
  ...
  await runImmediateScanIfRequested();
})();
...
function registerScheduledTriggers(): void {
  if (T1_ENABLED) {
...
async function runImmediateScanIfRequested(): Promise<void> {
